...
| [`03_Estimate_Contact_Agentic/`](03_Estimate_Contact_Agentic/) | Automatic iterative contact proposal, verification, correction, and mask export; its round-one masks define the reported single-shot ablation |
...
The repository includes prompts, intermediate representations, masks, optimized parameters, evaluation outputs, and multi-view renders for the benchmark. A useful end-to-end example is `interaction_22`:

- [input instruction and scene metadata](01_Generate_SIG/input_prompts/interaction_22/input_scene.json)
- [validated Scene Interaction Graph](01_Generate_SIG/output/interaction_22/sig.json)
- [agentic-loop summary](03_Estimate_Contact_Agentic/output/interaction_22/agentic_summary.json)
- [optimization summary](05_Optimize_Static_Scene/output/interaction_22/alignment_summary.json)
...
- Gemini API access for SIG generation, human-frame generation and selection, and contact evaluation; OpenAI API access for contact-overlay image editing
...
Store provider keys in the ignored files `.secrets/openai_api_key` and `.secrets/gemini_api_key`. Do not commit credentials. The active pipeline defaults to Gemini 3.7 Flash for SIG generation, human-frame selection, and contact evaluation; Gemini 3.1 Flash Image for human-frame generation; and GPT Image 2 for contact-overlay generation.

### Single-sample development path

From the repository root, run the complete five-stage pipeline for one interaction:

```bash
bash run_method.sh interaction_22
```

The runner regenerates the human-frame candidates and agentic contact masks, stops if a stage fails or contact verification does not accept the masks, and writes the final result under `05_Optimize_Static_Scene/output/interaction_22/`. It uses the active Python environment; set `PIPELINE_PYTHON` to another Python executable if needed. The API stages make new paid requests on each run.

To run stages separately, the treadmill example is shown below. Module 02 builds the prompt, generates five candidate frames, and selects the best one for downstream use.

```bash
INTERACTION=interaction_22

python 01_Generate_SIG/01_generate_sig.py --interaction_name "$INTERACTION"
python 02_Generate_Human_Frame/01_generate_human_frame.py --interaction_name "$INTERACTION"
python 03_Estimate_Contact_Agentic/01_estimate_agentic_contact.py --interaction_name "$INTERACTION"
...
For the single-shot ablation, run `python 03_Estimate_Contact_Agentic/02_build_round1_contact_masks.py --interaction_name "$INTERACTION"` after the automatic contact estimator.

The contact estimator reuses existing prepared assets and round images by default. Add `--overwrite` to rebuild the assets and regenerate contact overlays from round 1.

Contact estimation and GVHMR initialization are independent after human-frame generation and can be run in parallel. Every entry point exposes `--help` for path and model overrides.

Evaluation and rendering entry points in modules 06–09 select one interaction with `--interaction_name interaction_22`, or the full set with `--interaction_name all`. There is no `--all_interactions` alias in these entry points. Their `--output_root` is always the evaluation base directory: per-interaction artifacts go under `<output_root>/<interaction>/physical_plausibility`, `semantics`, or `vlm`. Diversity and aggregate reports go directly under the base directory. Single-interaction input overrides cannot be used with `all`.

The static optimizer writes `optimized_params.pt`, `loss_history.csv`, and `final_loss.csv` directly into its interaction output directory, alongside `meshes/` and `alignment_summary.json`. Add `--debug` to export depth images, iteration snapshots, SDF diagnostics, and loss plots. These diagnostics are disabled by default; the numerical loss terms and optimization stages are unchanged.

Scripts consume the current SIG schema (`target_objects` and `interaction_edges`) and the current optimizer output layout. Old schemas and output locations are not loaded automatically. VLM evaluation uses `--qwen_model` and `--prompt_template`; the old `--model` and `--system_prompt` aliases have been removed. The renderer uses `--max_views`; obsolete orbit-camera options and `--num_views` have been removed. Ablations still reuse the full-method cameras automatically.

```bash
python 06_Evaluate_Interaction/01_evaluate_physical_plausibility.py --interaction_name interaction_22
python 06_Evaluate_Interaction/03a_render_interaction.py --interaction_name all
python 06_Evaluate_Interaction/03_evaluate_semantics.py --interaction_name all
```

The frozen thesis results used Gemini 3.5 Flash for structured reasoning and evaluation, Nano Banana 2 for human-frame generation, GPT Image 2 for contact overlays, GPT-5.5 for contact verification, SAM 3 for segmentation, and GVHMR for pose initialization. The active pipeline defaults above differ from that historical configuration; model identifiers remain configurable in the development scripts.
