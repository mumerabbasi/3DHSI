#!/usr/bin/env python3
"""Render selected GenZI humans by replacing only the human in Module 06."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render a selected GenZI human in Module 06's exact Blender scene, "
...
def build_render_config(
    ...
    genzi_human = candidate.human_mesh_world.resolve()
    missing = [
        str(path)
        for path in (
            source_config_path,
    ...
    config.update(
        {
            ...
            "selected_candidate_dir": str(candidate.candidate_dir),
            "selected_smplx_params_world": str(candidate.smplx_params_world),
            ...
            "human_mesh_world": str(genzi_human),
            "blend_path": str(
                (output_root / "assets" / "render_scene.blend").resolve()
            ),
            "views": views,
            "coordinate_frame": "scannet_world",
            "human_replacement_only": True,
            "scene_and_camera_source": "Module 06 shared comparison render",
        }
    )
    return config
...
def render_interaction(
    ...
    save_json(
        ...
        {
            "interaction_name": interaction_name,
            "selected_candidate_index": config["selected_candidate_index"],
            ...
            "genzi_human_mesh_world": config["human_mesh_world"],
            "human_replacement_only": True,
            "views": config["views"],
        },
    )
    ...
    print(
        f"{interaction_name}: {action} {len(config['views'])} views "
        f"from GenZI candidate {config['selected_candidate_index']}"
    )
    return {
        "interaction_name": interaction_name,
        "selected_candidate_index": config["selected_candidate_index"],
...
def main() -> None:
    ...
    if len(names) > 1:
        save_json(
            genzi_eval_root(args.output_mode) / "semantics_renders.json",
