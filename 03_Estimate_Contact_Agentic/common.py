...
def contact_human_parts(
    ...
    requested = (
        normalize_scene_element(scene_element) if scene_element is not None else None
    )
    return list(
        dict.fromkeys(
            edge["human_part"]
            ...
            or normalize_scene_element(edge["scene_element"]) == requested
        )
    )
...
def contact_palette_from_spec(
    path: Path, human_parts: list[str]
) -> list[dict[str, Any]]:
    by_part = {item["part"]: item for item in load_json(path)["palette"]["parts"]}
    return [by_part[part] for part in human_parts]
...
def fit_bbox_to_image_aspect(
    ...
    if image_width <= 0 or image_height <= 0:
        raise ValueError(
            "Image dimensions must be positive to fit an aspect-ratio crop."
        )
    ...
    if x1 <= x0 or y1 <= y0:
        raise ValueError(f"Invalid bbox for aspect-ratio crop: {bbox}")

    ratio_gcd = math.gcd(image_width, image_height)
    aspect_unit_w = image_width // ratio_gcd
    aspect_unit_h = image_height // ratio_gcd
    bbox_w = x1 - x0
    bbox_h = y1 - y0
    crop_scale = max(
        math.ceil(bbox_w / aspect_unit_w),
        math.ceil(bbox_h / aspect_unit_h),
    )
    crop_w = min(image_width, crop_scale * aspect_unit_w)
    crop_h = min(image_height, crop_scale * aspect_unit_h)
