#!/usr/bin/env python3
"""Render PROX by replacing only the human in Module 06's shared blend."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render a PROX human in Module 06's exact Blender scene, changing "
            "only the human mesh."
        )
    )
...
def build_render_config(
    ...
    prox_human = (interaction_root / "final_smplx_world.ply").resolve()
    missing = [
        str(path)
        for path in (
            source_config_path,
    ...
    config.update(
        {
            ...
            "human_mesh_world": str(prox_human),
...
def render_interaction(
    ...
    interaction_root = prox_interaction_root(interaction_name, args.output_mode)
    ...
    save_json(
        selected_views_path,
        {
            ...
            "prox_human_mesh_world": config["human_mesh_world"],
            "human_replacement_only": True,
            "views": config["views"],
        },
    )
...
def main() -> None:
    ...
    if len(names) > 1:
        save_json(
            prox_eval_root(args.output_mode) / "semantics_renders.json",
